					...
					<tr class="dataRow">
						...
						<c:choose>
							<c:when test="${vo.ordNo >= 2 }">
								...
								<i class="material-icons md" style="font-size: 15px; color: #4b8a08;">subdirectory_arrow_right</i>
									${vo.title }
								</td>
							...
							<c:otherwise>
								<td class="tourTumbnail"><img src=${vo.tourThumbnail } class="tourThumbnail"><br>${vo.tourTitle }</td>
								<td class="rating">
								...
								<td style="text-align: left;">${vo.title }
								<c:if test="${vo.replyCnt > 0}"> [${vo.replyCnt }]</c:if></td>
								<td>${vo.nickname } (${vo.gradeName})</td>
								<td><fmt:formatDate value="${vo.writeDate }" pattern="yyyy-MM-dd HH:mm:ss" /></td>
